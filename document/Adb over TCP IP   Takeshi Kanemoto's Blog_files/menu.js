...
var GhostlyMenu=Class.create();GhostlyMenu.prototype={initialize:function(target,align,sub,offsetTop){this.obj=cleanWhitespace(target);this.align=align||'left';this.sub=sub||-1;this.offsetTop=offsetTop||0;this.menu=this.obj.childNodes
if(this.menu.length<2){return;}
this.title=this.menu[0];this.body=this.menu[1];cleanWhitespace(this.body).lastChild.getElementsByTagName('a')[0].className+=' last';setStyle(this.body,'visibility','hidden');setStyle(this.body,'display','block');addListener(this.obj,'mouseover',bind(this,this.activate),false);addListener(this.obj,'mouseout',bind(this,this.deactivate),false);},activate:function(){if(this.sub==1){var pos=currentOffset(this.title);var top=pos[1]-1;var left=getWidth(this.body)-2;if(this.align=='right'){var left=getWidth(this.body)*(-1);}}else{var pos=cumulativeOffset(this.title);if(document.body.className.indexOf('loggedin')!=-1){var top=pos[1]+(2*getHeight(this.title))-3;}else{var top=pos[1]+getHeight(this.title);}
top=top+this.offsetTop;var left=pos[0];if(this.align=='right'){var offset=getWidth(this.title)-getWidth(this.body);left+=offset;}}
...
var subscribe=document.getElementById('subscribe-feeds');if(subscribe){new GhostlyMenu(subscribe,align,-1,-5);}
